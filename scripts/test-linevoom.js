...
async function getRecentPostContainers(page) {
  const raw = await page.evaluate(() => {
    ...
    const all = [...document.querySelectorAll('body *')]; const result = [];
    for (const el of all) {
      ...
      if (!post.dataset.voomAuditId) post.dataset.voomAuditId = `audit-${result.length}-${Math.random().toString(36).slice(2)}`;
      result.push({ id: post.dataset.voomAuditId, timeLabel: text });
    }
    return result;
  });
  const now = new Date(); const seen = new Set(); const recent = [];
  for (const row of raw) {
    const publishedAt = parseRelativeTime(row.timeLabel, now);
    if (!publishedAt || publishedAt < CUTOFF || seen.has(row.id)) continue;
    seen.add(row.id); recent.push({ ...row, publishedAt });
  }
  return recent.sort((a, b) => b.publishedAt - a.publishedAt);
}

async function expandRecentPosts(page) {
...
async function parseItemsFromPostDom(page) {
  const candidates = await page.evaluate(() => [...document.querySelectorAll('[data-voom-audit-id]')].map((post) => {
    ...
    const items = links.map((link) => {
      let name = ''; let node = link.nextSibling;
      while (node) { if (node.nodeType === Node.TEXT_NODE) name += node.textContent || ''; else if (node.nodeName === 'BR') break; else name += node.textContent || ''; node = node.nextSibling; }
  ...
  candidates.sort((a, b) => b.items.length - a.items.length); const deduped = new Map();
  for (const item of candidates[0].items) { const cleaned = { name: cleanName(item.name), url: item.url }; deduped.set(`${productCode(cleaned.name) || cleaned.name}|${cleaned.url}`, cleaned); }
...
async function getLatestGiveaway(page) {
  const posts = await page.evaluate(() => {
    ...
    for (let i = 0; i < lines.length - 1; i += 1) { if (lines[i] !== 'Public' || !timePattern.test(lines[i + 1])) continue; const segment = lines.slice(start, i + 2); result.push({ text: segment.join('\n'), timeLabel: lines[i + 1] }); start = i + 2; }
    return result;
  });
  const now = new Date(); const parsedPosts = [];
  for (const post of posts) { const publishedAt = parseRelativeTime(post.timeLabel, now); if (!publishedAt || publishedAt < CUTOFF) continue; parsedPosts.push({ ...post, publishedAt, textLength: post.text.length }); }
  parsedPosts.sort((a, b) => b.publishedAt - a.publishedAt || a.textLength - b.textLength);
  if (!parsedPosts.length) return null;
  const latestRelevant = parsedPosts.find(({ text }) => GIVEAWAY_KEYWORDS.some((keyword) => text.includes(keyword))); if (!latestRelevant) return null;
  let items = parseItems(latestRelevant.text);
  if (!items.length) { items = await parseItemsFromPostDom(page); if (items.length) console.log(`🧩 DOM fallback 解析到 ${items.length} 個商品連結`); }
