...
function extractStartTime(text=''){const flat=normalizeRange(text.replace(/\r/g,' ').replace(/\n+/g,' '));const labels='(?:抽選\\s*[/&＆]?\\s*購買時間|抽選時間|抽籤時間(?:&使用期限)?|抽獎時間|購買時間|抽選期間|抽籤期間|購買期間|購買資格券?有效期間|優惠券有效期間)';const date='(?:20\\d{2}\\s*[\\/.-]\\s*)?\\d{1,2}\\s*[\\/.-]\\s*\\d{1,2}';const clock='(?:\\d{1,2}\\s*:\\s*\\d{2}|開店)';const re=new RegExp(`(${labels})\\s*[:：]?\\s*(${date}\\s+${clock})\\s*(?:-|至|到)\\s*(${date}\\s+${clock})`,'i');const m=flat.match(re);if(!m)return null;return `${m[1].replace(/\s+/g,'')} ${m[2].replace(/\s+/g,' ')} - ${m[3].replace(/\s+/g,' ')}`}
...
// Rewrite the complete store object. Remove every existing startTime first, then insert exactly one before items.
// This also repairs DATA already corrupted by an older run that appended startTime after items[].
...
(async()=>{let src=fs.readFileSync(DATA_FILE,'utf8'),data=parseData(src);let updated=0,detected=0,missing=0;log(`⏰ LINE VOOM startTime ${SYNC?'SYNC':'AUDIT'} — ${STORES.length} 間`);log('📅 規則：只看 2026/09/01 後最新相關文章，完整時間才同步；每間店永遠只保留一個 startTime');log(`📄 完整結果：${OUTPUT_FILE}`);const browser=await chromium.launch({headless:false});const ctx=await browser.newContext({locale:'zh-TW',timezoneId:'Asia/Taipei'}),page=await ctx.newPage();try{for(let i=0;i<STORES.length;i++){const s=STORES[i];log(`\n[${i+1}/${STORES.length}] ${s.region} / ${s.name}`);const ds=findStore(s,data);if(!ds){missing++;log('⏭️ DATA 無對應店家；NEW STORE sync 後再跑即可補上');continue}try{await page.goto(s.url,{waitUntil:'domcontentloaded',timeout:60000});await page.waitForTimeout(2200);const more=page.getByText('顯示更多',{exact:false});for(let n=0;n<Math.min(await more.count(),10);n++)try{if(await more.nth(n).isVisible())await more.nth(n).click({timeout:1000})}catch{}const ps=await recentPosts(page);const relevant=ps.filter(p=>KEYWORDS.some(k=>p.text.includes(k)));if(!relevant.length){log('— 9/1 後無相關文章');continue}const latest=relevant[0];const found=extractStartTime(latest.text);if(!found){log(`— 最新相關文章 ${latest.label} 未偵測到完整抽選/購買時間範圍`);continue}detected++;log(`⏰ ${found}`);if(!SYNC)continue;const next=setStartTime(src,ds.name,found);if(next===src){log('⚠️ 找到時間但 DATA block 未更新');continue}src=next;updated++;log(`💾 startTime 已覆蓋：${found}`);data=parseData(src)}catch(e){log(`❌ ${e.message}`)}}}finally{await browser.close();if(SYNC&&updated)fs.writeFileSync(DATA_FILE,src,'utf8');log(`\nSUMMARY: detected=${detected} updated=${updated} missing=${missing}`);save()}})().catch(e=>{log(`FATAL: ${e.stack||e}`);save();process.exitCode=1});