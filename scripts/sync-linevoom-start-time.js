...
(async()=>{let src=fs.readFileSync(DATA_FILE,'utf8'),data=parseData(src);let updated=0,detected=0,missing=0;log(`⏰ LINE VOOM startTime ${SYNC?'SYNC':'AUDIT'} — ${STORES.length} 間`);log('📅 規則：文章發布時間只用來挑選近期文章；startTime 直接解析文章內文的完整抽選/購買日期時間範圍');log('🛡️ 無法確認完整日期＋時間時保留原 startTime，不覆蓋');log(`📄 完整結果：${OUTPUT_FILE}`);const browser=await chromium.launch({headless:false});const ctx=await browser.newContext({locale:'zh-TW',timezoneId:'Asia/Taipei'}),page=await ctx.newPage();try{for(let i=0;i<STORES.length;i++){const s=STORES[i];log(`\n[${i+1}/${STORES.length}] ${s.region} / ${s.name}`);const ds=findStore(s,data);if(!ds){missing++;log('⏭️ DATA 無對應店家；NEW STORE sync 後再跑即可補上');continue}try{await page.goto(s.url,{waitUntil:'domcontentloaded',timeout:60000});await page.waitForTimeout(2200);const more=page.getByText('顯示更多',{exact:false});for(let n=0;n<Math.min(await more.count(),10);n++)try{if(await more.nth(n).isVisible())await more.nth(n).click({timeout:1000})}catch{}const ps=await recentPosts(page);const relevant=ps.filter(p=>KEYWORDS.some(k=>p.text.includes(k)));if(!relevant.length){log('— 9/1 後無相關文章');continue}const latest=relevant[0];const found=extractStartTime(latest.text);if(!found){log(`⚠️ 找到文章 ${latest.label}，但無法確認完整抽選時間，startTime 不更新`);continue}detected++;log(`⏰ 文章內文解析：${found}`);if(!SYNC)continue;if(ds.startTime===found){log('✅ startTime 已一致，不需更新');continue}const next=setStartTime(src,ds.name,found);if(next===src){log('⚠️ 找到時間但 DATA block 未更新');continue}src=next;updated++;log(`💾 startTime 已覆蓋：${found}`);data=parseData(src)}catch(e){log(`❌ ${e.message}`)}}}finally{await browser.close();if(SYNC&&updated)fs.writeFileSync(DATA_FILE,src,'utf8');log(`\nSUMMARY: detected=${detected} updated=${updated} missing=${missing}`);save()}})().catch(e=>{log(`FATAL: ${e.stack||e}`);save();process.exitCode=1});